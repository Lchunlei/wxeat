...
Page({
  ...
  onLoad: function (e) {
    ...
    if (!e.scene) {
      ...
      if (e.inviter_id) {
        console.log("inviter_id页面跳转" + e.inviter_id);
        console.log("ductId页面跳转" + e.ductId);
        wx.setStorage({
      ...
      success: function (res) {
        ...
        if (res.data.respData.properties) {
          for (var i = 0; i < res.data.respData.properties.length; i++) {
            selectSizeTemp = selectSizeTemp + " " + res.data.respData.properties[i].proName;
          }
          that.setData({
            hasMoreSelect: true,
  ...
  labelItemTap: function (e) {
    ...
    for (var i = 0; i < that.data.goodsDetail.properties.length; i++) {
      ...
      for (var j = 0; j < childs.length; j++) {
        if (childs[j].active) {
          curSelectNum++;
          propertyChildIds = propertyChildIds + that.data.goodsDetail.properties[i].proId + ":" + childs[j].spuId + ",";
          propertyChildNames = propertyChildNames + that.data.goodsDetail.properties[i].proName + ":" + childs[j].spuName + "  ";
        }
      }
    }
    var canSubmit = false;
    ...
    if (canSubmit) {
      ...
      console.log("Id-->" + that.data.goodsDetail.basicInfo.ductId);
      wx.request({
        // url: app.siteInfo.url + app.siteInfo.subDomain + '/goods/price',
        url: app.globalData.urls + '/goods/price',
        data: {
          goodsId: that.data.goodsDetail.basicInfo.ductId,
        ...
        success: function (res) {
          ...
          that.setData({
            selectSizePrice: res.data.respData.price,
            propertyChildIds: propertyChildIds,
            propertyChildNames: propertyChildNames,
            buyNumMax: res.data.respData.stores,
            buyNumber: (res.data.respData.stores > 0) ? 1 : 0,
            selectptPrice: res.data.respData.price
          });
